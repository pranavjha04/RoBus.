...
@WebServlet("/upload_operator_logo.do")
public class UploadOperatorLogoServlet extends HttpServlet {
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        try {
            ...
            for(FileItem next : items) {
                if(next.getFieldName().equals("logo")) {
                    target = next;
                    break;
                }
            }
            ...
            File oldLogoFile = null;

            File[] files = uploadDir.listFiles();
            if (files != null) {
                for (File oldImageFile : files) {
                    if (oldImageFile.getName().contains(operator.getLogo())) {
                        oldLogoFile = oldImageFile;
                        break;
                    }
                }
            }
            ...
            if (oldLogoFile != null) {
                ...
                if (!deleted) {
                    throw new Exception("Old logo could not be deleted");
                }
            }
            ...
            boolean isUpdated = Operator.updateLogo(fileName, operator.getOperatorId());
            if(!isUpdated) throw new Exception();

            operator.setLogo(fileName);
