...
@WebServlet("/upload_operator_banner.do")
public class UploadOperatorBannerServlet extends HttpServlet {
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        try {
            ...
            for(FileItem next : items) {
                if(next.getFieldName().equals("banner")) {
                    target = next;
                    break;
                }
            }
            ...
            File oldBannerFile = null;

            File[] files = uploadDir.listFiles();
            if (files != null) {
                for (File oldImageFile : files) {
                    if (oldImageFile.getName().contains(operator.getBanner())) {
                        oldBannerFile = oldImageFile;
                        break;
                    }
                }
            }
            ...
            if (oldBannerFile != null) {
                boolean deleted = oldBannerFile.delete();

                if (!deleted) {
                    throw new Exception("Old Banner could not be deleted");
                }
            }
            ...
            boolean isUpdated = Operator.updateBanner(fileName, operator.getOperatorId());
            if(!isUpdated) throw new Exception();

            operator.setBanner(fileName);
